...
const VERDICT_STYLE = {
  'Strong Candidate':          { bg: '#DCFCE7', text: '#166534', icon: '✅', pill: { text: '#166534', bg: '#DCFCE7', border: '#86EFAC' } },
  'Promising Candidate':       { bg: '#DBEAFE', text: '#1E40AF', icon: '👍', pill: { text: '#1E40AF', bg: '#DBEAFE', border: '#93C5FD' } },
  'Viable — Review Cautions':  { bg: '#FEF3C7', text: '#92400E', icon: '⚠️', pill: { text: '#92400E', bg: '#FEF3C7', border: '#FCD34D' } },
  'Not Recommended':           { bg: '#FEE2E2', text: '#991B1B', icon: '⛔', pill: { text: '#991B1B', bg: '#FEE2E2', border: '#FCA5A5' } },
};

function ScorePill({ score, verdict }) {
  const vs = VERDICT_STYLE[verdict]?.pill || { text: '#6B7280', bg: '#F3F4F6', border: '#E5E7EB' };
  return (
    <span
      className="text-xs font-bold px-2.5 py-1 rounded-lg border tabular-nums"
      style={{ color: vs.text, backgroundColor: vs.bg, borderColor: vs.border }}
    >
...
function StarRow({ rating }) {
  ...
  return (
    <span style={{ color: '#F59E0B', fontSize: 13, letterSpacing: 1 }}>
      {'★'.repeat(r)}<span style={{ color: '#D1D5DB' }}>{'★'.repeat(5 - r)}</span>
    </span>
  );
}
                ...
                <div className="flex items-start gap-2.5 mb-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-1.5 mt-1 flex-wrap">
                      <StarRow rating={pick.star_rating} />
                      {pick.verdict && <VerdictBadge verdict={pick.verdict} />}
                    </div>
                    {pick.address && (
                  ...
                  <ScorePill score={pick.score} verdict={pick.verdict} />
                </div>
